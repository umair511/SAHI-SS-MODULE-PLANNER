...
import { JumboRequirement, SSMachineSettings, JumboRoll } from '../../types/ss';
import { isSSOrder, generateSSJumboRollRequirements } from '../../services/ss/ssOptimizer';
import { 
  fetchSSJumboRequirementsAsync, 
  fetchPS01PlansAsync,
  getStoredActiveJob,
  resumeActiveSynthesisJob,
  clearStoredActiveJob
} from '../../services/ss/ssApi';
import { 
...
export const SSJumboRequirementsView: React.FC<SSJumboRequirementsViewProps> = ({
  ...
  const handleDeleteSingleReq = (reqId: string, planIndex?: number) => {
    const planTitle = planIndex !== undefined ? `SS Plan #${planIndex + 1}` : 'this SS requirement plan';
    setDeleteConfirmModal({
      isOpen: true,
      title: 'Delete SS Requirement Plan',
  ...
  const handleDeleteAllForFilm = () => {
    ...
    setDeleteConfirmModal({
      isOpen: true,
      title: 'Delete All SS Requirement Plans',
      message: `Are you sure you want to cancel and delete all ${count} SS requirement plans for ${selectedFilm}?`,
  ...
  const handleGenerate = async () => {
    ...
    setGenerationProgress('Filtering secondary slitter orders & analyzing film compatibility...');
  ...
  // Send manually selected plans to SS Planning Studio & physical stock
  const handleSendSelectedToMSL = () => {
    ...
    if (selectedList.length === 0) {
      alert('Please select at least one approved plan to send to SS Planning Studio.');
      return;
    }
    ...
    try {
      ...
      // Synchronize physical jumbo rolls into inventory for SS Studio (deduplicated by deterministic key)
  ...
  return (
    <div className="space-y-6" id="ss-requirements-view">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
          ...
          <div className="bg-slate-50 p-3 rounded-lg border border-slate-200">
            ...
            <span className="text-slate-700">
              1–14 UPS available · 20–30mm trim target · Max Dia 1000mm · 6" Paper Core
            </span>
          </div>
        </div>
      </div>
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs">
                <span className="text-xs font-bold uppercase tracking-wider text-slate-500">Selected SS Sourcing</span>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <button
                  ...
                  title={`Cancel & Delete all ${filteredReqs.length} generated SS plans for ${selectedFilm}`}
                >
                ...
                >
                  ...
                  <span>{isSendingToMsl ? 'Staging into SS...' : `Send Selected Plans to SS Studio (${selectedCount})`}</span>
                </button>
              </div>
            </div>
      ...
      {/* Success Notification Banner for SS Sending */}
      {mslSentSuccess && (
          ...
          <div className="flex items-center space-x-3 text-xs font-medium">
            ...
            <div>
              <p className="font-bold text-sm text-emerald-950">
                {mslSentSuccess.count} Plan(s) Successfully Sent to SS Planning Studio!
              </p>
              <p className="text-emerald-800 mt-0.5">
                Staged {mslSentSuccess.totalKg.toLocaleString()} KG of approved jumbo requirements into active SS inventory at {mslSentSuccess.timestamp}.
              </p>
            </div>
          </div>
          <button
        ...
        {filteredReqs.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-xl p-12 text-center text-slate-400 space-y-3">
            ...
            <p className="text-xs max-w-md mx-auto text-slate-500">
              Click "Synthesize & Handshake" above to explore 1–14 UPS configurations and perform the automatic feasibility check with PS01.
            </p>
            <button
              onClick={handleGenerate}
              ...
              return (
                <div className="bg-white border border-slate-200 rounded-xl p-10 text-center text-slate-500 space-y-3 shadow-xs">
                  ...
                  <p className="text-sm font-bold text-slate-800">
                    No {feasibilityFilter} SS Plans Found for {selectedFilm}
                  </p>
                  <p className="text-xs text-slate-500">
                ...
                {deckleGroups.map((group, groupIdx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* 2. DOWNSTREAM SECONDARY SLITTER (SS) PLANS PRODUCED FROM THIS MOTHER RUN */}
                              ...
                              >
                                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 pb-3 border-b border-slate-100">
                                  <div className="space-y-1">
                                    <div className="flex flex-wrap items-center gap-2">
                                      {/* Individual SS Plan Selection Checkbox */}
                                      <button
                                        ...
                                        title={isReqRed ? 'Infeasible plans cannot be selected' : isReqSelected ? 'Click to Deselect' : 'Click to Select for SS'}
                                      >
                                        {isReqSelected ? (
                                          <CheckSquare className="w-3.5 h-3.5" />
                                        ) : isReqRed ? (
                                  ...
                                  {/* Right side quick specs & View SS Sheet button */}
                                  <div className="flex flex-wrap items-center gap-3">
                                    ...
                                    <div className="flex items-center space-x-1.5">
                                      ...
                                      <button
                                        ...
                                        title={`Cancel & Delete SS Plan #${originalIndex + 1}`}
                                      >
                                ...
                                {/* Yellow Trim Relaxation Acceptance for this SS Plan if needed */}
                                {isReqYellow && (
