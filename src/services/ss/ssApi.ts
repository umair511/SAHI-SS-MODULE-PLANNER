...
const inFlightSynthesis = new Map<string, Promise<SSJumboRequirement[]>>();
const inFlightPlans = new Map<string, Promise<{ plans: SSPlan[]; remainingOrders: VA05Order[]; updatedRolls: SSJumboRoll[] }>>();
const inFlightPS01 = new Map<string, Promise<{ plans: SlitterPlan[]; logs: any[] }>>();

const ACTIVE_SS_JOB_KEY = 'ss_active_synthesis_job';

export function getStoredActiveJob(): { jobId: string; film: string; timestamp: number } | null {
  try {
    const raw = sessionStorage.getItem(ACTIVE_SS_JOB_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw);
    if (Date.now() - parsed.timestamp < 10 * 60 * 1000) {
      return parsed;
    }
    sessionStorage.removeItem(ACTIVE_SS_JOB_KEY);
    return null;
  } catch {
    return null;
  }
...
async function pollJobUntilDone<T>(
  ...
  while (Date.now() - startTime < maxWaitMs) {
    try {
      const res = await fetch(`/api/ss/jobs/${jobId}`);
      if (res.ok) {
        consecutiveErrors = 0;
        ...
        if (data.success && data.job) {
          ...
          if (status === 'FAILED') {
            throw new Error(error || 'Server SS optimization job failed');
          }
        }
      ...
      if (err.message && (err.message.includes('Server SS optimization job failed') || err.message.includes('Job was not found'))) {
        throw err;
      }
      consecutiveErrors++;
  ...
  throw new Error('Server SS optimization job exceeded maximum polling timeout (30s)');
}

export async function fetchSSJumboRequirementsAsync(
  orders: VA05Order[],
  settings: SSMachineSettings,
  film?: string,
  onProgress?: (update: JobProgressUpdate) => void
): Promise<SSJumboRequirement[]> {
  ...
  const promise = (async () => {
    onProgress?.({ percent: 5, stage: 'Connecting to server SS optimizer...' });

    const res = await fetch('/api/ss/synthesize-requirements', {
      method: 'POST',
    ...
    try {
      sessionStorage.setItem(ACTIVE_SS_JOB_KEY, JSON.stringify({
        jobId,
        film: film || 'ALL',
        timestamp: Date.now(),
      }));
    ...
    const jobResult = await pollJobUntilDone<{ requirements: SSJumboRequirement[]; count: number }>(
      jobId,
      onProgress
    );
...
export const fetchJumboRequirementsAsync = fetchSSJumboRequirementsAsync;
...
export async function fetchSSPlansAsync(
  orders: VA05Order[],
  ...
  const promise = (async () => {
    onProgress?.({ percent: 10, stage: 'Initializing SS plan optimizer on server...' });

    const res = await fetch('/api/ss/generate-plans', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    const jobId = initData.jobId;
    if (!jobId) {
      throw new Error('Server did not return a valid SS job identifier');
    }
...
export const fetchMetallizerPlansAsync = fetchSSPlansAsync;
...
export async function fetchPS01PlansAsync(
  ...
  const promise = (async () => {
    ...
    const res = await fetch('/api/ss/generate-ps01-plans', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
